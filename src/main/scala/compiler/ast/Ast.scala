...
object Ast {
  ...
  case class Module(nameSpace: NameSpace, imports: Seq[Import], models: Seq[Model])

  case class Import(loc: Seq[String])

  case class Field(name: String, `type`: Type, init: Option[Type])

  case class Type(ref: Ref)

  trait Ref
  case class RefSpecial(specialRef: SpecialRef) extends Ref
  case class RefLocal(name: String) extends Ref
  ...
  case class QualName(nameSpace: NameSpace, name: String)
  ...
  case class CurlyBraceBlock(statements: Seq[Statement]) extends Block
  ...
  trait Statement
  case class ClassModel(name: String, modifiers: Seq[Modifier], fields: Seq[Field], parent: Option[Type], parentArguments: Seq[Expression], interfaces: Seq[Type], statements: Seq[Statement]) extends Model
  ...
  case class Method(name: String, annotations: Seq[Annotation], fields: Seq[Field], modifiers: Seq[Modifier], returnType: Option[Type], body: Block) extends Statement
  ...
  case class Assign(name: String, `type`: Option[Type], immutable: Boolean, block: Block) extends Statement
