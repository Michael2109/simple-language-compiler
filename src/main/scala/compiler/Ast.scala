...
object AST {
  ...
  case class File(header: ModuleHeader, models: Seq[Model])
  ...
  case class CurlyBraceBlock(statement: Seq[Statement]) extends Block
  ...
  trait Statement
  case class ClassModel(name: String, modifiers: Seq[Modifier], fields: Seq[Field], parent: Option[Type], parentArguments: Seq[Expression], interfaces: Seq[Type], body: Seq[Statement]) extends Model
